...
public class RadioBandContainerMarshaller
extends AbstractContainerMarshaller {
    public RadioBandContainerMarshaller() {
        super(30);
    }
    ...
    public HASDataContainer marshal(HASContainer hASContainer) {
        ...
        if (null == hASContainer) {
            ...
            RadioBandContainer radioBandContainer = (RadioBandContainer)hASContainer;
            ArrayList arrayList = RadioBandContainerMarshaller.borrowArrayList();
            if (radioBandContainer.isBandSet()) {
                RadioBandContainerMarshaller.add((List)arrayList, RadioBandContainerMarshaller.createIntElement(65, radioBandContainer.getBand()));
            }
            if (0 < arrayList.size()) {
                hASDataContainer.dataElements = (HASDataElement[])arrayList.toArray(new HASDataElement[arrayList.size()]);
            }
            RadioBandContainerMarshaller.release(arrayList);
        }
    ...
    protected void setIntElement(HASContainer hASContainer, int n, int n2) {
        switch (n) {
            case 65: {
                ((RadioBandContainer)hASContainer).setBand(n2);
                break;
            }
            default: {
