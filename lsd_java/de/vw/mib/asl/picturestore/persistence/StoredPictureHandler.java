...
class StoredPictureHandler
extends AbstractPersistenceDataHandler {
    private final StoredPicture handledPersistable;
    private final PersistenceModule module;
    static final /* synthetic */ boolean $assertionsDisabled;
    static /* synthetic */ Class class$de$vw$mib$asl$picturestore$persistence$StoredPictureHandler;

    StoredPictureHandler(StoredPicture storedPicture, Logger logger, PersistenceModule persistenceModule) {
        super(logger);
        if (!$assertionsDisabled && storedPicture == null) {
            throw new AssertionError();
        }
        this.handledPersistable = storedPicture;
    ...
    public void handle(int n, int n2) {
        switch (n) {
            case 1: {
                this.handledPersistable.setIdentification(n2);
                this.markAsProcessed(n);
                break;
            }
            case 3: {
                this.handledPersistable.setBitfieldOfUsers(n2);
                this.markAsProcessed(n);
                break;
            }
            case 4: {
                this.handledPersistable.setCtxId(n2);
                this.markAsProcessed(n);
                break;
            }
            default: {
    ...
    public void handle(int n, String string) {
        switch (n) {
            case 2: {
                this.handledPersistable.setUrl(string);
                this.markAsProcessed(n);
                break;
            }
            default: {
    ...
    static {
        $assertionsDisabled = !(class$de$vw$mib$asl$picturestore$persistence$StoredPictureHandler == null ? (class$de$vw$mib$asl$picturestore$persistence$StoredPictureHandler = StoredPictureHandler.class$("de.vw.mib.asl.picturestore.persistence.StoredPictureHandler")) : class$de$vw$mib$asl$picturestore$persistence$StoredPictureHandler).desiredAssertionStatus();
    }
}
