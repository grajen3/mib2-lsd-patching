...
class PictureStoreHandler
extends AbstractPersistenceDataHandler {
    private final PictureStore handledPersistable;
    private final PersistenceModule module;
    static final /* synthetic */ boolean $assertionsDisabled;
    static /* synthetic */ Class class$de$vw$mib$asl$picturestore$persistence$PictureStoreHandler;

    PictureStoreHandler(PictureStore pictureStore, Logger logger, PersistenceModule persistenceModule) {
        super(logger);
        if (!$assertionsDisabled && pictureStore == null) {
            throw new AssertionError();
        }
        this.handledPersistable = pictureStore;
    ...
    public void handle(int n, Persistable persistable) {
        switch (n) {
            case 1: {
                this.handledPersistable.getPictures().add(persistable);
    ...
    static {
        $assertionsDisabled = !(class$de$vw$mib$asl$picturestore$persistence$PictureStoreHandler == null ? (class$de$vw$mib$asl$picturestore$persistence$PictureStoreHandler = PictureStoreHandler.class$("de.vw.mib.asl.picturestore.persistence.PictureStoreHandler")) : class$de$vw$mib$asl$picturestore$persistence$PictureStoreHandler).desiredAssertionStatus();
    }
}
