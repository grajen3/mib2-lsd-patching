...
public final class StateSetAmBandRange
extends AbstractState {
    private Logger logger = ServiceManager.logger;
    private static final long TIMEOUT;

    StateSetAmBandRange(StateMachineOwner stateMachineOwner, String string, HsmState hsmState) {
    ...
    public HsmState handle(EventGeneric eventGeneric) {
        ...
        switch (eventGeneric.getReceiverEventId()) {
            case 3: {
                ...
                this.mOwner.getAmfmTuner().setAMBandRange(n);
                break;
            }
            default: {
    ...
    public void dsiAMFMTunerSetAMBandRangeStatus(int n) {
        AmFmProxyHelper.setAMBandRangeStatus(this.mOwner.getTargetId(), n);
        if (n != 1) {
            this.getRequest().setResult(0);
            this.getRequest().setInt(1, n);
            if (this.logger.isTraceEnabled(128)) {
                this.logger.trace(128).append(RadioUtil.LOG_PREFIX_AMFM_TUNER).append("set AM band range status: ").append(n).log();
            }
