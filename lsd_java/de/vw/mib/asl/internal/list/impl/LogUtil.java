...
final class LogUtil {
    ...
    static LogMessage appendListId(LogMessage logMessage, int n, boolean bl) {
        return LogUtil.appendIntProperty(logMessage, "listId", n, bl);
    }

    static LogMessage appendListName(LogMessage logMessage, String string, String string2) {
        if (null != string2) {
            logMessage.append(" (").append(string).append(".").append(string2).append(")");
        }
        return logMessage;
    }

    static LogMessage appendListConfig(LogMessage logMessage, ASLList aSLList, boolean bl) {
        if (aSLList.isFrozen()) {
            LogUtil.appendString(logMessage, "FROZEN", bl);
            bl = true;
        }
        if (aSLList.isModCountEnabled()) {
            LogUtil.appendModCount(logMessage, aSLList.getModCount(), bl);
            bl = true;
        }
        LogUtil.appendListId(logMessage, aSLList.getListId(), bl);
        return LogUtil.appendListName(logMessage, aSLList.getDeviceName(), aSLList.getName());
    }
