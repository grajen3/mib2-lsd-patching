...
extends AbstractHsmState {
    ...
    public StateContactsSearchHandlingSearchHandling(HsmMessagingContactsSearchHandling hsmMessagingContactsSearchHandling, GenericEvents genericEvents, Hsm hsm, String string, HsmState hsmState) {
    ...
    public HsmState handle(EventGeneric eventGeneric) {
        ...
        switch (eventGeneric.getReceiverEventId()) {
            case 1074641836: {
                AbstractASLHsmTarget.writeBooleanToDatapool(-1159590656, true);
                ListManager.getGenericASLList(469).updateStringIndex(ASLPhoneFactory.getPhoneApi().getDefaultCharacterInfo().getIndices(), ASLPhoneFactory.getPhoneApi().getDefaultCharacterInfo().getCharacters());
                boolean bl = MessagingUtil.updateAddressbookSize(this.target.phoneContactsSize);
            ...
            case 1074641842: {
                ...
                if (this.target.selectionStart == this.target.selectionEnd) {
                    ...
                    AbstractASLHsmTarget.writeSpellerDataToDatapool(-1176367872, this.target.spellerData);
                    if (string.length() > 1) {
            ...
            case 1074641833: {
                ...
                if (n2 != -1) {
                    if (this.currentSpellerTypeDSI != n2) {
                        if (MessagingSearchFetcher.spellerHandle <= -1 && MessagingSearchFetcher.spellerHandle != -4) {
                            this.target.dsiAdbList.startSpeller(1, 20, n2);
                            this.target.spellerData.setAvailableCharacters("");
                            MessagingSearchFetcher.spellerHandle = -2;
                        } else {
                            this.target.restartSpellerMode = n2;
                            this.target.restartSpeller = true;
                            if (MessagingSearchFetcher.spellerHandle != -4) {
                                this.stopSpeller();
                            }
                        }
                    } else {
            ...
            case 1073744609: 
            case 1074641829: {
                ListManager.getGenericASLList(469).updateStringIndex(ASLPhoneFactory.getPhoneApi().getCharacterInfos().getIndices(), ASLPhoneFactory.getPhoneApi().getCharacterInfos().getCharacters());
                AbstractASLHsmTarget.writeBooleanToDatapool(-1159590656, false);
            ...
            case 1074641844: {
                ...
                ASLMessagingData.hwrResult = (DataSet)ListManager.getGenericASLList(-1058927360).getDSIObjects()[n];
                AbstractASLHsmTarget.writeBooleanToDatapool(-1159590656, true);
                MessagingSearchFetcher.spellerHandle = -3;
                ListManager.getGenericASLList(469).setSize(0);
                ListManager.getGenericASLList(469).setSize(1);
                break;
            }
            default: {
    ...
    private void stopSpeller() {
        switch (MessagingSearchFetcher.spellerHandle) {
            case -4: 
            case -2: 
            case -1: {
                this.target.restartSpeller = false;
            ...
            case -3: {
                this.target.restartSpeller = false;
                this.dsiAdbListStopSpellerResult(0, 0);
                break;
            }
            default: {
                ...
                this.target.dsiAdbList.stopSpeller(MessagingSearchFetcher.spellerHandle);
                this.target.spellerData.setAvailableCharacters("");
                MessagingSearchFetcher.spellerHandle = -4;
    ...
    public void dsiAdbListSpellerResult(int n, int n2, DataSet[] dataSetArray, int n3, String string, String string2) {
        ...
        if (n == 0 && (n2 == MessagingSearchFetcher.spellerHandle || MessagingSearchFetcher.spellerHandle == -2)) {
            MessagingSearchFetcher.spellerHandle = n2;
            MessagingSearchFetcher.spelleredListSize = n3;
            ...
            if (!Util.isNullOrEmpty(dataSetArray)) {
                ...
                if (this.target.isHWRActive) {
                    if (this.target.spellerData.getEnteredText().length() == 0) {
                        ListManager.getGenericASLList(-1058927360).clear();
                    } else {
                        ListManager.getGenericASLList(-1058927360).updateList(dataSetArray);
                    }
                }
            } else {
    ...
    public void dsiAdbListStopSpellerResult(int n, int n2) {
        ...
        if (this.target.restartSpeller) {
            ...
            MessagingSearchFetcher.spellerHandle = -1;
        }
        MessagingUtil.updateAddressbookSize(0);
        MessagingUtil.updateAddressbookSize(this.target.phoneContactsSize);
    }
    ...
    private void writeSpellerChar(String string) {
        ...
        if (bl2) {
            this.target.operationPending = true;
            this.target.dsiAdbList.addSpellerChars(MessagingSearchFetcher.spellerHandle, string);
        } else if (bl) {
            this.dsiAdbListSpellerResult(0, MessagingSearchFetcher.spellerHandle, this.target.southsideDataSets, this.target.southsideTotalHits, this.target.southsideValidChars, this.target.southsideUniqueChars);
        } else {
            ++this.target.invalidCharacters;
            this.dsiAdbListSpellerResult(0, MessagingSearchFetcher.spellerHandle, new DataSet[0], 0, "", new StringBuffer().append(this.target.uniqueChars).append(string).toString());
        }
    }
    ...
    private void removeSpellerChar() {
        if (this.target.invalidCharacters > 0) {
            if (--this.target.invalidCharacters == 0) {
                ...
                this.dsiAdbListSpellerResult(0, MessagingSearchFetcher.spellerHandle, new DataSet[0], 0, "", this.target.uniqueChars.substring(0, this.target.uniqueChars.length() - 1));
            }
        } else {
            this.target.operationPending = true;
