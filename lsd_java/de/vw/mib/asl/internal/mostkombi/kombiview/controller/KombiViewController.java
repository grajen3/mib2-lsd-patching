...
import de.vw.mib.asl.internal.mostkombi.kombiview.controller.KombiViewCategoryManager;
...
public class KombiViewController
extends AbstractASLHsmTarget
implements KombiViewHsmContextDelegate,
Controller,
CategoryManagerDelegate {
    private ControllerDelegate _delegate;
    private KombiViewHsmContext _hsmContext;
    private KombiViewCategoryManager _categoryManager;
    private boolean _initialized;

    public KombiViewController(GenericEvents genericEvents, int n, String string) {
    ...
    private KombiViewHsmContext getHsmContext() {
        if (this._hsmContext == null) {
            this._hsmContext = new KombiViewHsmContext(this.getHsm(), this.getWorkStateParent());
            this._hsmContext.setDelegate(this);
        }
        return this._hsmContext;
    }

    private KombiViewCategoryManager getCategoryManager() {
        if (this._categoryManager == null) {
            this._categoryManager = new KombiViewCategoryManager(this);
        }
