...
CioRegistrationListener {
    ...
    private void initCioIntents(CioIntent cioIntent) {
        ...
        this.systemServices.getCioFactory().createCio(this.systemServices.getAppName(), this.systemServices.getAppInstanceName());
        cioCreator.setPayloadData(cioIntent.getUsage());
        CioIntent cioIntent3 = cioCreator.finishCallbackCioIntent(this);
        CioActionParameters cioActionParameters = this.systemServices.getCioFactory().createActionParameters(cioIntent).setCioIntent("HidePushupAction", cioIntent2).setCioIntent("ShowPopupAction", cioIntent3).finishActionParams();
    ...
    public void execute(ContentInformationObject contentInformationObject, CioActionParameters cioActionParameters) {
        if (contentInformationObject.getActionId() == 0) {
            ...
            if ("LeftSeatMenu".equals(string)) {
                ServiceManager.eventDispatcher.createAndSubmitHMIEvent(865875968);
            } else if ("RightSeatMenu".equals(string)) {
                ServiceManager.eventDispatcher.createAndSubmitHMIEvent(882653184);
            } else {
