...
public class MediaControlTimer
implements Runnable {
    ...
    public MediaControlTimer(Runnable runnable, int n, String string) {
    ...
    public MediaControlTimer(Runnable runnable, String string) {
        this.mRunnable = runnable;
        this.mTimerId = string;
        if (threadSwitchingTarget == null) {
            threadSwitchingTarget = ASLFrameworkFactory.getASLFrameworkAPI().getServices().getASLThreadSwitch();
    ...
    private void trace(String string) {
        if (ServiceManager.logger.isTraceEnabled(8)) {
            ServiceManager.logger.trace(8).append("MEDIA CONTROL TIMER: ").append(string).append(" \"").append(this.mTimerId).append('\"').log();
        }
    }

    private void error(String string) {
        ServiceManager.logger.error(8).append("MEDIA CONTROL TIMER: ").append(string).append(" \"").append(this.mTimerId).append('\"').log();
    }
}
