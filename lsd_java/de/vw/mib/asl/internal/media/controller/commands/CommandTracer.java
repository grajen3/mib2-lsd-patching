...
public class CommandTracer {
    ...
    public Logger getLogger() {
        ...
        if (logger == DEFAULT_COMMAND_LOGGER && n == 1024) {
            return this.mLogger;
        }
        return logger;
    }
    ...
    public void logAborted(int n, boolean bl, String string, boolean bl2) {
        if (bl2) {
            if (this.isTraceEnabled()) {
                this.warn(new StringBuffer().append("CANCELED ").append(this.mCommand.getCommandStr(true, false, false, false)).append(this.getOptionalDetails(string)).toString());
            }
        } else {
    ...
    protected void logStep(int n, int n2, int n3) {
        if (this.isTraceEnabled()) {
            ...
            String string2 = " |";
            this.trace(new StringBuffer().append(" ").append(string).append(this.getStepStr(n, true, false)).append(" |").append(" ").append(this.getStepStr(n, false, true)).toString());
        }
    }
    ...
    protected void logException(Exception exception) {
        if (exception != null) {
            ServiceManager.logger.error(1024).attachThrowable(exception).log();
        }
    }
