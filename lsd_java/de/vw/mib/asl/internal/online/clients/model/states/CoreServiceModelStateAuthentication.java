...
public class CoreServiceModelStateAuthentication
extends AbstractHsmState {
    private final CoreServiceModelTarget _target;
    private final String _classname = super.getClass().getName();
    private int _subCategoryID;

    public CoreServiceModelStateAuthentication(CoreServiceModelTarget coreServiceModelTarget, Hsm hsm, String string, HsmState hsmState) {
        super(hsm, string, hsmState);
        this._target = coreServiceModelTarget;
        if (this.getTarget().isTraceEnabled()) {
            LogMessage logMessage = this.getTarget().trace();
            logMessage.append(this._classname).append(".CoreServiceModelStateAuthentication()").log();
        }
    }
        ...
        switch (eventGeneric.getReceiverEventId()) {
            ...
            case 100005: {
                ...
                if (!Util.isNullOrEmpty(string)) {
                    if (string.equalsIgnoreCase(CoreServiceModelMap.getServiceIDWithPreCheckID(44, 0))) {
                        CoreServiceModelFactory.getCoreServiceModelController(this.getTarget()).getModelHandler().setInternalPrecheckStatus(n == 0 ? 0 : 1);
                    } else {
