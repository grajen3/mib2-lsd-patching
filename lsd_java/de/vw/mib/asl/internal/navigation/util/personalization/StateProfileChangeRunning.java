...
public class StateProfileChangeRunning
extends AbstractHsmState {
    private IExtLogger logger = ASLNavigationUtilFactory.getNavigationUtilApi().getExtLogger(16384, "[StateProfileChangeRunning]");
    private HsmTargetProfileChange target;

    public StateProfileChangeRunning(HsmTargetProfileChange hsmTargetProfileChange, String string, HsmState hsmState) {
    ...
    public HsmState handle(EventGeneric eventGeneric) {
        switch (eventGeneric.getReceiverEventId()) {
            ...
            case 100101: {
                ...
                this.handlePostDsiProfileChange(navProfileChangeHandler);
                break;
            }
            default: {
    ...
    private void handlePostDsiProfileChange(NavProfileChangeHandler navProfileChangeHandler) {
        this.logger.trace("handlePostDsiProfileChange()");
        ASLNavigationUtilFactory.getNavigationUtilApi().getNavigationDp().getPersonalNaviPersistence().unlockPersitence();
        this.target.profileChangeHandler = navProfileChangeHandler;
        this.target.transStatePostDsiForProfileChange();
    }
