...
public class TrafficStateMapMessageDetails
extends AbstractHsmState
implements ASLListElementFetcher,
CacheClient {
    private final IExtLogger logger = ASLNavigationUtilFactory.getNavigationUtilApi().getExtLogger(16, "[TrafficStateMapMessageDetails]");
    ...
    public TrafficStateMapMessageDetails(TrafficTargetHSM trafficTargetHSM, Hsm hsm, String string, HsmState hsmState) {
    ...
    public void dsiTmcOnRouteTmcMessage(TmcMessage tmcMessage) {
        ...
        if (this.logger.isTraceEnabled()) {
            ASLNavigationUtilFactory.getNavigationUtilApi().getExtLoggerHelper().appendTmcMessage(this.logger.makeTrace(), tmcMessage).log();
        }
        ...
        if (tmcMessage == null || genericASLList == null || genericASLList2 == null) {
            block4: {
                this.logger.trace("dsiTmcOnRouteTmcMessage() (null == tmcMessage) || (null == tmcOverviewList) || (null == tmcDetailList)!");
                try {
                    this.target.stateMain.transStateMain();
                }
                catch (Exception exception) {
                    if (!this.logger.isTraceEnabled()) break block4;
                    this.logger.makeError().append("dsiTmcOnRouteTmcMessage()").append(exception.toString()).log();
                }
            }
            return;
        }
        ...
        tmcListElement = this.target.createTmcListElementByEventText(tmcMessage);
        ...
        this.modelNotifier.notifyObjectTypesOnMap(1);
    ...
    private TrafficMessageIcon getTmcIcon(TmcListElement tmcListElement) {
        ...
        if (tmcListElement.message.iconListId != null && tmcListElement.message.iconListId.length > 0) {
            ...
            this.logger.makeTrace().append("getTmcIcon(uID=").append(tmcListElement.uID).append(") no iconList").log();
        }
    ...
    private void handleEntry(EventGeneric eventGeneric) {
        ...
        if (genericASLList == null) {
            this.logger.error("handleEntry() tmcOverviewList null!");
            return;
        }
    ...
    private void requestTmcMessageDetails(EventGeneric eventGeneric) {
        try {
            ...
            if (l < Integer.MIN_VALUE || l > 0) {
                this.logger.makeWarn().append("tmcId=").append(l).append(", out of range").log();
                this.target.stateMain.transStateMain();
            } else {
