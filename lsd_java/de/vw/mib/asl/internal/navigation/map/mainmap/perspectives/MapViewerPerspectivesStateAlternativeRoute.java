...
public final class MapViewerPerspectivesStateAlternativeRoute
extends AbstractHsmState {
    private final IExtLogger logger = ASLNavigationUtilFactory.getNavigationUtilApi().getExtLogger(256, "[MapViewerPerspectivesStateAlternativeRoute]");
    private final MapViewerPerspectivesTargetHSM target;

    public MapViewerPerspectivesStateAlternativeRoute(MapViewerPerspectivesTargetHSM mapViewerPerspectivesTargetHSM, String string, HsmState hsmState) {
    ...
    private void handleDefault(EventGeneric eventGeneric) {
        if (this.logger.isTraceEnabled()) {
            this.logger.makeTrace().append("handleDefault(event=").append(eventGeneric.toString()).append(")").log();
        }
    }
    ...
    private void handleExit() {
        ...
        this.target.datapool.setPerspectiveAlternativeRouteMap(false);
        this.target.notifierModel.notifyAlternativeRouteMapInactive();
    }
    ...
    private void handleStart() {
        ...
        this.target.datapool.setPerspectiveAlternativeRouteMap(true);
        this.target.notifierDSI.viewFreezeTrue();
        this.target.notifierDSI.setModeAlternativeRouteMap();
        ...
        this.target.notifierModel.notifyAlternativeRouteMapActive();
