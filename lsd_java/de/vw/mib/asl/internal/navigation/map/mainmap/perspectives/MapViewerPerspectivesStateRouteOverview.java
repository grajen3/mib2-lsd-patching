...
public final class MapViewerPerspectivesStateRouteOverview
extends AbstractHsmState {
    private final IExtLogger logger = ASLNavigationUtilFactory.getNavigationUtilApi().getExtLogger(256, "[MapViewerPerspectivesStateRouteOverview]");
    private final MapViewerPerspectivesTargetHSM target;

    public MapViewerPerspectivesStateRouteOverview(MapViewerPerspectivesTargetHSM mapViewerPerspectivesTargetHSM, String string, HsmState hsmState) {
    ...
    private void handleExit() {
        ...
        this.target.datapool.setPerspectiveRouteOverviewActive(false);
    }

    private void handleStart() {
        this.logger.trace("handleStart()");
        this.target.datapool.setPerspectiveRouteOverviewActive(true);
        this.target.notifierDSI.viewFreezeTrue();
        this.target.notifierDSI.setModeRouteOverview();
