...
public class NumberSpellerFilterAdapter
...
implements NavGatewayListener {
    ...
    public void filterCharacters(String[] stringArray) {
        ...
        if (stringArray != null && stringArray.length > 0) {
            ...
            stringBuffer.append("Entering NumberSpellerFilter.filteredCharacters -> character is: ").append(stringArray[0]);
            this.logger.trace(stringBuffer.toString());
            new CmdGetMatchingNVC(this, stringArray[0]).execute();
        } else {
            this.logger.trace("HWR did not recognize any character");
    ...
    public void handleCommandCmdGetMatchingNVC(CmdGetMatchingNVC cmdGetMatchingNVC) {
        ...
        StringBuffer stringBuffer = new StringBuffer("Entering HWR: handleCommandCmdGetMatchingNVC. Recognized character from southside: ");
        stringBuffer.append(this.matchingNVCs);
        this.logger.trace(stringBuffer.toString());
        String[] stringArray = new String[1];
        if (cmdGetMatchingNVC.validNvc.length() > 0) {
            stringArray[0] = cmdGetMatchingNVC.validNvc;
        } else {
