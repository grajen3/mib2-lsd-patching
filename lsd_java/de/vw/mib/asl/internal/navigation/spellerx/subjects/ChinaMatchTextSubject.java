...
public class ChinaMatchTextSubject
extends NavReflectionSubject {
    ...
    public ChinaMatchTextSubject(XSpellerTarget xSpellerTarget, int n, int n2, ModelListHandler modelListHandler, ModelListHandler modelListHandler2, int n3, boolean bl, boolean bl2, boolean bl3) {
    ...
    public void evtHSM_START(ISpellerContext iSpellerContext) {
        ...
        if (SpellerContext.cancelFlag && this.target.getSpellerContext().getActiveField().hasDSIState() || StateZipCode.restore) {
            this.restoreSpeller();
    ...
    public void evtEV_SET_INPUT(ISpellerContext iSpellerContext, String string) {
        this.mtsUtil.printTraceMessage(this.target, new StringBuffer("CHINAMATCHTEXTSUBJECT:evtEV_SET_INPUT: Setting string: ").append(string).toString());
        new CmdSetInput(this, string, false).execute();
    }

    public void evtEV_SELECT_VIA_INDEX(ISpellerContext iSpellerContext, int n) {
        new FreeTextSubjectUtil().makeTraceComment(this.target, new StringBuffer("CHINAMATCHTEXTRSUBJECT:evtEV_SELECT_VIA_INDEX: Selecting index: ").append(n).toString());
    ...
    public void evtEV_SELECT_VIA_ID(ISpellerContext iSpellerContext, String string) {
        this.mtsUtil.printTraceMessage(this.target, new StringBuffer().append("CHINAMATCHTEXTRSUBJECT: Received event evtEV_SELECT_VIA_ID with objectId:").append(string).toString());
        if (string != null) {
            this.freezeChnLists();
    ...
    public void evtASL_NAVIGATION_LOCATIONINPUT_INPUTFIELD_SELECT_NOTHING(ISpellerContext iSpellerContext) {
        this.mtsUtil.printTraceMessage(this.target, "CHINAMATCHTEXTRSUBJECT: evtASL_NAVIGATION_LOCATIONINPUT_INPUTFIELD_SELECT_NOTHING entered");
        this.freezeChnLists();
    ...
    public void evtASL_NAVIGATION_LOCATIONINPUT_INPUTFIELD_FILTER_BY_ADD_CHARACTER_HWR(ISpellerContext iSpellerContext, String string, int n, int n2, int n3) {
        ...
        if (!SpellerContext.hwrEnabled) {
            ...
            this.target.getSpellerContext().getActiveField().getSpellerData().setCursorPosition(this.enteredString.length());
    ...
    public void evtASL_NAVIGATION_LOCATIONINPUT_INPUTFIELD_FILTER_BY_UNDO_LAST_CHARACTERS(ISpellerContext iSpellerContext) {
        ...
        if (this.characterQueue.length() > 0) {
            ...
            this.mtsUtil.printTraceMessage(this.target, "CHINAMATCHTEXTSUBJECT.evtASL_NAVIGATION_LOCATIONINPUT_INPUTFIELD_FILTER_BY_UNDO_LAST_CHARACTERS: Removing character from southside ( via CmdUndoChar)");
            this.setShortValueListMode();
    ...
    public void handleCommandCmdSelectListItem(CmdSelectListItem cmdSelectListItem) {
        this.mtsUtil.printTraceMessage(this.target, "CHNMATCHTEXTSUBJECT.handleCommandCmdSelectListItem() called");
        ...
        if (SpellerContext.ACTIVE_STATE == 6) {
            this.triggerCrosshairMode(cmdSelectListItem.liCurrentState_liCurrentLD);
        }
    }

    private void triggerCrosshairMode(NavLocation navLocation) {
        EventGeneric eventGeneric = ServiceManager.mGenericEventFactory.newEvent(1172386560);
        eventGeneric.setObject(0, navLocation);
        this.target.triggerObserver(1172386560, eventGeneric);
    }

    @Override
    public void handleCommandCmdSelectListItemByIdent(CmdSelectListItemByIdent cmdSelectListItemByIdent) {
        this.mtsUtil.printTraceMessage(this.target, "CHNMATCHTEXTSUBJECT.handleCommandCmdSelectListItemByIdent() called");
    ...
    public void handleCommandCmdSetInput(CmdSetInput cmdSetInput) {
        if (cmdSetInput.liResult_ReturnCode != 0L) {
            this.target.getLogger().error("CHNMATCHTEXTSUBJECT->handleCommandCmdSetInput southside return value is NOK");
            return;
        }
        ...
        if (this.characterQueue.length() > 0) {
            this.mtsUtil.printTraceMessage(this.target, new StringBuffer("CHNMATCHTEXTSUBJECT->handleCommandCmdSetInput Sending (adding) characters by sending cmdSetinput to southside. String: ").append(this.enteredString).toString());
            new CmdSetInput(this, this.enteredString, false).execute();
            this.characterQueue = "";
            this.addCharCommandSend = true;
            this.updateSpellerWithAllCharactersAvailable();
        ...
        this.mtsUtil.printTraceMessage(this.target, "CHNMATCHTEXTSUBJECT->handleCommandCmdSetInput: No more chars in queue. Updating model");
