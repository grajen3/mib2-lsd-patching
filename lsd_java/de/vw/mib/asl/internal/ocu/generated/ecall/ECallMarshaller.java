...
implements BAPRequestMarshaller {
    ...
    public ECallMarshaller(BAPService bAPService, BitStreamTransformer bitStreamTransformer) {
    ...
    public void startResult(int n, BAPEntity bAPEntity) {
        switch (n) {
            case 18: {
                this.getBapService().request(n, 0, 4, this.getBitstreamTransformer().toInteger(bAPEntity, ECallMarshaller.getBitstreamTransformerDatatype(0)));
                break;
            }
            case 19: {
                this.getBapService().requestVoid(n, 4);
                break;
            }
            case 25: {
    ...
    public void setGetEntity(int n, BAPEntity bAPEntity) {
        switch (n) {
            case 13: {
                this.getBapService().request(n, 0, 0, this.getBitstreamTransformer().toInteger(bAPEntity, ECallMarshaller.getBitstreamTransformerDatatype(0)));
                break;
            }
            case 16: {
    ...
    public void ackEntity(int n, BAPEntity bAPEntity) {
        switch (n) {
            case 16: {
                this.getBapService().requestByteSequence(n, 6, this.getBitstreamTransformer().toByteArray(bAPEntity));
                break;
            }
            case 17: {
                this.getBapService().requestByteSequence(n, 6, this.getBitstreamTransformer().toByteArray(bAPEntity));
