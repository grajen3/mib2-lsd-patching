...
class SDSSD_ASGCapabilitiesHandler
extends AbstractPersistenceDataHandler {
    private final SDSSD_ASGCapabilities handledPersistable;
    private final PersistenceModule module;
    static final /* synthetic */ boolean $assertionsDisabled;
    static /* synthetic */ Class class$de$vw$mib$asl$instrumentcluster$persistence$SDSSD_ASGCapabilitiesHandler;

    SDSSD_ASGCapabilitiesHandler(SDSSD_ASGCapabilities sDSSD_ASGCapabilities, Logger logger, PersistenceModule persistenceModule) {
        super(logger);
        if (!$assertionsDisabled && sDSSD_ASGCapabilities == null) {
            throw new AssertionError();
        }
        this.handledPersistable = sDSSD_ASGCapabilities;
    ...
    public void handle(int n, int n2) {
        switch (n) {
            case 1: {
                this.handledPersistable.setInstrumentClusterDisplaySize(n2);
    ...
    protected void installDefaultValue(int n) {
        switch (n) {
            case 1: {
                this.handledPersistable.setInstrumentClusterDisplaySize(1);
                break;
            }
            default: {
    ...
    static {
        $assertionsDisabled = !(class$de$vw$mib$asl$instrumentcluster$persistence$SDSSD_ASGCapabilitiesHandler == null ? (class$de$vw$mib$asl$instrumentcluster$persistence$SDSSD_ASGCapabilitiesHandler = SDSSD_ASGCapabilitiesHandler.class$("de.vw.mib.asl.instrumentcluster.persistence.SDSSD_ASGCapabilitiesHandler")) : class$de$vw$mib$asl$instrumentcluster$persistence$SDSSD_ASGCapabilitiesHandler).desiredAssertionStatus();
    }
}
